...
module.exports = async (message) => {
  ...
  if(clientdb.get("word") === false) return;
    ...
    let miktar = clientdb.get(`kufur_${message.author.id}`);
    if(miktar >= clientdb.get("warnlimit")) {
        clientdb.del(`kufur_${message.author.id}`)
...
await message.channel.send(`${message.member} küfür kullanımı yasak olmasına rağmen **${clientdb.get("warnlimit")}** kere kullandığı için **${time.replace("m", " Dakika").replace("h", " Saat").replace("d", " Gün")}** cezalıya atıldı!`).then(x => x.delete({timeout: 10000}));
setTimeout(async() => {
...
if(type == "mute") {
  let punishperm = message.guild.roles.cache.get(clientdb.get("punishrole"));
  await message.member.roles.add(punishperm);
  await message.channel.send(`${message.member} küfür kullanımı yasak olmasına rağmen **${clientdb.get("warnlimit")}** kere kullandığı için **${time.replace("m", " Dakika").replace("h", " Saat").replace("d", " Gün")}** susturuldu!`).then(x => x.delete({timeout: 10000}));
  setTimeout(async() => {
  await message.member.roles.remove(punishperm)
  }, ms(time)) 
}
    ...
    message.delete().then(async() => {
      await clientdb.s.add(`kufur_${message.author.id}`, 1)
      await message.channel.send(`${message.member} Küfür kullanımı yasaktır!`).then(x => x.delete({timeout: 10000}));
  })
}
